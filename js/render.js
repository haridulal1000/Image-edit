...
function renderDrawLayer(layer) {
    ...
    renderImage.width = layer.width;
    renderImage.height = layer.height;
    renderImage.style = `filter: brightness(${
    layer.brightness | 100
  }%) contrast(${layer.contrast | 100}%) hue-rotate(${layer.hue}deg) saturate(${
    layer.saturation
  }%) blur(${layer.blur}px) invert(${layer.invert}%) grayscale(${
    layer.grayscale
  }%) sepia(${layer.sepia}%)`;
...
function renderLineLayer(layer) {
    ...
    parseFloat(layer.point.x1) +
    parseFloat(layer.strokeWeight)
  }" height="${
    Math.abs(parseFloat(layer.point.y2) - parseFloat(layer.point.y1)) +
    parseFloat(layer.point.y1) +
    parseFloat(layer.strokeWeight)
  }">
      <line  x1="${layer.point.x1}" y1="${layer.point.y1}" x2="${
    layer.point.x2
  }" y2="${layer.point.y2}" stroke="${layer.stroke}" stroke-width="${
    layer.strokeWeight
  }"/>
    </svg>
    `;
...
function renderCircleLayer(layer) {
    ...
    div.innerHTML = ` <svg  width="${
    layer.radius * 2 + layer.strokeWeight
  }" height="${layer.radius * 2 + layer.strokeWeight}">
      <circle stroke-opacity="${strokeOpacity}" fill-opacity="${fillOpacity}" cx="${
    layer.radius + layer.strokeWeight / 2
  }" cy="${layer.radius + layer.strokeWeight / 2}" r="${layer.radius}" fill="${
    layer.fill
  }" stroke="${layer.stroke}" stroke-width="${layer.strokeWeight}"/>
    </svg>
    `;
    div.style.opacity = parseFloat(layer.opacity) / 100;
    document.getElementById("imageView").appendChild(div);
...
function renderRectLayer(layer) {
    ...
    div.innerHTML = ` <svg  width="${
    parseFloat(layer.width) + parseFloat(layer.strokeWeight)
  }" height="${parseFloat(layer.height) + parseFloat(layer.strokeWeight)}">
      <rect stroke-opacity="${strokeOpacity}" fill-opacity="${fillOpacity}" x="${
    layer.strokeWeight / 2
  }" y="${layer.strokeWeight / 2}" width="${layer.width}" height="${
    layer.height
  }" fill="${layer.fill}" stroke="${layer.stroke}" stroke-width="${
    layer.strokeWeight
  }"/>
    </svg>
    `;
