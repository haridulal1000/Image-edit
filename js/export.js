...
function Export() {
    ...
    for (let i = 0; i < layers.length; i++) {
        ...
        if (layers[i].type === "text" && layers[i].visible === true) {
            exportText(layers[i]);
        }

        if (layers[i].type === "line" && layers[i].visible === true) {
            exportLine(layers[i]);
        }

        if (layers[i].type === "circle" && layers[i].visible === true) {
            exportCircle(layers[i]);
        }

        if (layers[i].type === "rect" && layers[i].visible === true) {
            exportRect(layers[i]);
        }

        if (layers[i].type === "polygon" && layers[i].visible === true) {
    ...
    cropContext.quadraticCurveTo(
        x + width,
        y,
        x + width,
        y + parseFloat(cropRadius)
    );
    cropContext.lineTo(x + width, y + height - parseFloat(cropRadius));
    cropContext.quadraticCurveTo(
        x + width,
        y + height,
        x + width - parseFloat(cropRadius),
        y + height
    );
    ...
    cropContext.quadraticCurveTo(
        ...
        y + height,
        x,
        y + height - parseFloat(cropRadius)
    );
...
function exportImage(layer) {
    ...
    context.save();
    context.translate(
        parseFloat(parseFloat(layer.x) - cropX) + parseFloat(layer.originX),
        parseFloat(parseFloat(layer.y) - cropY) + parseFloat(layer.originY)
    );
    ...
    context.drawImage(
        layer.image,
        parseFloat(layer.x) - cropX,
        parseFloat(layer.y) - cropY,
        layer.width,
        layer.height
    );
    context.restore();
...
function exportText(layer) {
    ...
    for (let i = 0; i < textArray.length; i++) {
        context.fillText(
            textArray[i],
            parseFloat(layer.x) - cropX,
            parseFloat(layer.y) - cropY + parseInt(layer.fontSize) * (i + 1)
        );
    }
...
function exportCircle(layer) {
    ...
    let centerX =
        parseInt(parseFloat(layer.x) - cropX) +
        parseInt(layer.radius) +
        parseFloat(layer.strokeWeight) / 2;
    let centerY =
        parseInt(parseFloat(layer.y) - cropY) +
        parseInt(layer.radius) +
        parseFloat(layer.strokeWeight) / 2;
...
function exportLine(layer) {
    ...
    context.moveTo(x, y);
    context.lineTo(
        layer.point.x2 + 2 * layer.strokeWeight,
        layer.point.y2 + 2 * layer.strokeWeight
    );
    context.stroke();
    context.restore();
}
